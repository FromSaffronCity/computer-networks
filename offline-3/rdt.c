...
int is_any_message_in_transit = 0;

/* shared information */
struct pkt packet_to_transmit;
struct pkt nack;
...
void A_output(struct msg message) {
    printf("A_output: message received from layer5.\n");
    if(is_any_message_in_transit == 1) {
        printf("A_output: another message in transit. dropping this message.\n");
        return ;
    }
    int i;

    packet_to_transmit.seqnum = current_seqnum_A;
    packet_to_transmit.acknum = acknum_expected_A;
    for(i=0; i<20; i++) packet_to_transmit.payload[i] = message.data[i];
    packet_to_transmit.checksum = calculate_checksum(packet_to_transmit);

    is_any_message_in_transit = 1;
    starttimer(SENDER, 15.0);

    printf("A_output: packet passed to layer3.\n");
    tolayer3(SENDER, packet_to_transmit);
}

/* need to be completed only for extra credit */
void B_output(struct msg message) {
    printf("B_output: B does not send message. ignore.\n");
}

/* called from layer 3, when a packet arrives for layer 4 at A */
void A_input(struct pkt packet) {
    printf("A_input: packet received from layer3.\n");
    if(packet.checksum != calculate_checksum(packet)) {
        printf("A_input: packet corrupted. ACK dropped.\n");
    } else {
        if(packet.acknum != acknum_expected_A) {
            printf("A_input: packet acknum mismatched. ACK dropped.\n");
        } else {
            printf("A_input: packet acknum matched. ACK accepted.\n");
            current_seqnum_A = acknum_expected_A = (current_seqnum_A==0? 1: 0);
            is_any_message_in_transit = 0;
            stoptimer(SENDER);
        }
    }
}
...
void A_timerinterrupt(void) {
    printf("A_timerinterrupt: A's timer went off. retransmitting packet.\n");
    starttimer(SENDER, 15.0);

    printf("A_timerinterrupt: packet passed to layer3.\n");
    tolayer3(SENDER, packet_to_transmit);
}
...
void A_init(void) {
    printf("A_init: initialization for sender A.\n");
    current_seqnum_A = acknum_expected_A = 0;
}
...
void B_input(struct pkt packet) {
    printf("B_input: packet received from layer3.\n");
    int i;

    if(packet.checksum != calculate_checksum(packet)) {
        printf("B_input: packet corrupted. NACK passed to layer3.\n");
        tolayer3(RECEIVER, nack);
    } else {
        if(packet.seqnum != seqnum_expected_B) {
            printf("B_input: packet seqnum mismatched. NACK passed to layer3.\n");
            tolayer3(RECEIVER, nack);
        } else {
            nack.seqnum = packet.seqnum;
            nack.acknum = packet.acknum;
            for(i=0; i<20; i++) nack.payload[i] = packet.payload[i];
            nack.checksum = packet.checksum;

            seqnum_expected_B = current_acknum_B = (current_acknum_B==0? 1: 0);
            printf("B_input: packet seqnum matched. message passed to layer5. ACK passed to layer3.\n");
            tolayer5(RECEIVER, packet.payload);
            tolayer3(RECEIVER, nack);
        }
    }
}
...
void B_timerinterrupt(void) {
    printf("B_timerinterrupt: B does not have a timer. ignore.\n");
}

/* the following routine will be called once (only) before any other */
/* entity B routines are called. You can use it to do any initialization */
void B_init(void) {
    printf("B_init: initialization for receiver B.\n");
    int i;

    seqnum_expected_B = current_acknum_B = 0;
    nack.seqnum = nack.acknum = 1;
    for(i=0; i<20; i++) nack.payload[i] = 'Z';
...
struct event {
    ...
    struct pkt *pktptr;  /* ptr to packet (if any) associated with this event */
    struct event *prev;
    struct event *next;
};
...
int main(void) {
    ...
    while(1) {
        ...
        if(eventptr->evtype == FROM_LAYER5) {
            ...
            if(eventptr->eventity == A)  /* deliver packet by calling */ A_input(pkt2give);  /* appropriate entity */
...
void init()  /* initialize the simulator */ {
    ...
    scanf("%f", &lossprob);
    printf("Enter packet corruption probability [enter 0.0 for no corruption]: ");
    scanf("%f", &corruptprob);
...
void tolayer3(int AorB, struct pkt packet) {
    ...
    if(jimsrand() < corruptprob) {
        ncorrupt++;
        if((x = jimsrand()) < .75) mypktptr->payload[0] = 'Z';  /* corrupt payload */
